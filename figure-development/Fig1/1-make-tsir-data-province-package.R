...
dat.sum <- ddply(dat,.(provname, year, week_report), summarise, cases=sum(case))
head(dat.sum)
...
build.prov.tsir <- function(prov.case.dat, nat.births, nat.pop, prop.prov){
  ...
  #if there is an NA in cases, it means this was not reported for this timestep
  #set to 1
...
sort(unique(tsir.prov$time[tsir.prov$provname=="Battambang" & trunc(tsir.prov$time)==2014]-trunc(tsir.prov$time[tsir.prov$provname=="Battambang"& trunc(tsir.prov$time)==2014])))

tsir.prov$year <- as.factor(tsir.prov$year)
ggplot(tsir.prov) + geom_line(aes(x=biweek, y=cases, color=year)) + facet_wrap(provname~., scales = "free_y")

tsir.prov$year <- as.numeric(as.character(tsir.prov$year))

#some years have only 25 depending
