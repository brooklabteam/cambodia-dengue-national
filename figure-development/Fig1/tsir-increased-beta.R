...
sim.com.epi <- function(time.start, dat, epiyr, family){
  ...
  for(i in 1:length(fracincvec)){
    ...
    betaepiInc<-fracincvec[i]*betabegin[length(betabegin)]
  ...
  S_noinc<-Sepi
  beta_noinc<-betaepi
  #dat.out <- cbind.data.frame(epidemic_year=epiyr, epibetanoic= beta_noinc, frac_incbetabeta = fracincout)
...
Sdat2007 <- sim.return.S.incS(fracincS = sim.2007$frac_incbeta,
                              dat=tsir.dat,
                              time.start =  min(tsir.dat$time),
                              family="gaussian",
                              epiyr = 2007)
...
Sdat2012 <- sim.return.S.incS(fracincS = sim.2012$frac_incbeta,
...
#now return and simulate with results
Sdat2019 <- sim.return.S.incS(fracincS = sim.2019$frac_incbeta,
...
Sdat2019fullsim <- sim.return.S.noinc(fracincS = sim.all$frac_incbeta,
...
ggsave(file =paste0(homewd,"/SusFig-draft-births.png"),
       plot=p1,
       units=c("mm"),  
       width=80, 
       height=40, 
...
ggsave(file =paste0(homewd,"/SusFig-draft-time.png"),
       plot=p2,
       units=c("mm"),  
       width=80, 
       height=40, 
...
comp.2007 <- sim.with.increaseS(
  fracincS = sim.2007$frac_incbeta,
  dat = tsir.dat,
  time.start =  min(tsir.dat$time),
  family = "gaussian",
  epiyr = 2007
...
comp.2012 <- sim.with.increaseS(
  fracincS = sim.2012$frac_incbeta,
...
comp.2019 <- sim.with.increaseS(
  fracincS = sim.2019$frac_incbeta,
  dat = subset(tsir.dat, time >= min(tsir.dat$time[tsir.dat$time >=
                                                     2013]) & time < 2020),
  time.start =  min(tsir.dat$time[tsir.dat$time >=
                                    2013]),
