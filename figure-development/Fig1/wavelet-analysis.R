...
prov.rank.multi <- function(df2, df1){
  ...
  df2add <- dplyr::select(df.merge, timee, coherence_multi)
...
get.wavelet.dat <- function(dat, dat.all){
  ...
  #ggplot(dat) + geom_line(aes(x=time, y=multi_period))
 ...
 # power.mat <- power.table*power.table.pval
  ...
  #(f) the average wavelet coherency with temperature for that province - annual only
  ...
  prov.coher.sum.annual <- ddply(dat.prov.coherence.annual, .(time), summarise, num_coherence=sum(sig_notsig, na.rm = T), N_tot=length(unique(coherence_prov)))
...
get.wavelet.dat.sub <- function(dat, dat.all){
  ...
  dat$multi_period_max <-  c(unlist(max.period.list))
...
get.wavelet.sub.old <- function(dat, dat.all){
...
get.wavelet.sub2.old <- function(dat, dat.all){
