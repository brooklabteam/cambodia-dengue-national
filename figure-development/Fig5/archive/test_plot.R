...
#load(paste0(homewd,"/figure-development/Fig5/comp-dat-sim-test.Rdata"))
load(paste0(homewd,"/figure-development/Fig5/comp-dat-sim-test-7-9.Rdata"))
load(paste0(homewd,"/figure-development/Fig5/comp-dat-sim-lci-7-9.Rdata"))
load(paste0(homewd,"/figure-development/Fig5/comp-dat-sim-uci-7-9.Rdata"))
...
#supplement first
comp.dat.test$hyp <- as.character(comp.dat.test$hyp)
...
column.1.noCI <- function(dat, year.start){
...
col1 <- column.1.noCI(dat =comp.dat.test,  year.start =2002)
...
col2 <- column.2(dat =comp.dat.test, year.start = 2002)
#col2 <- column.2(dat =test.dat, year.start = 2002)
...
col3 <- column.3(dat =comp.dat.test,  year.start =2002)


#and put them together

Fig5test <- cowplot::plot_grid(col1, col2, col3, nrow = 1, ncol=3, labels=c("A", "B", "C"),rel_widths = c(1.15,1,1),  label_size = 22, align = "hv", label_x = c(0,-.03,0))

ggsave(filename = paste0(homewd, "/final-figures/test-plot-standardFOI.png"),
       plot = Fig5test,
       units="mm",  
       width=110, 
       height=150, 
