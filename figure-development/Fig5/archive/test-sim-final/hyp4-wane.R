...
log.lik.wane.fit.all <- function(par, par.dat, dat){
  ...
  year.start = min(par.dat$year)
  out.mod <- model.age.incidence.series.wane(par.dat= par.dat, year.start=year.start, age_vect=age_vect)
  ...
  #ggplot(data=out.merge) + geom_line(aes(x=age,y= cum_prop_cases)) + geom_line(aes(x=age,y= cum_prop_cases_data), color="tomato") + facet_wrap(~year)
...
fit.wane.LBFGSB.all.three <- function(dat.all,par.dat, burnin, perc.obs, sigma.guess, fit.CI, sim_type){
...
load("cam-sim-infecteds.Rdata")

cam.sim = subset(cam.sim.infecteds, hyp=="4")


hyp4.fit.wane <- fit.wane.LBFGSB.all.three(dat.all = cam.sim,
                                           burnin=60,
                                           par.dat=hyp4.fit.lambda,
                                           sigma.guess = 1/50,
                                           sim_type="hyp4_wane", 
                                           perc.obs = 0.1,
                                           fit.CI=T)

save(hyp4.fit.wane, file = "hyp4-fit-wane.Rdata")
