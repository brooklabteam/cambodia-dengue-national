...
buildTMat_four_strain <- function(c, Npop, age.classes, surv.biwk, wane_hetero, age.brk, foi, age.mult.foi, recov, age.rate, pre.vect){
  ...
  #multiply by the age.mult vector - this is the biweekly probability of infection
  foi <- foi*age.mult.foi 
  # However, because it included an infected population when originally estimated from Muench,
  # we need to modulate it here for the local scenario --
  # so we will multiply by the proportion infected with the serotype of interest 
  # for each transmission event.
  Npop_split = chunk(Npop, c) 
  Npop_tot = Reduce('+', (Npop_split)) 
  #this is the total number within #each epidemic class across all ages
  #now get the proportion infected for each serotype
  nfoi1 <- sum(Npop_tot[2],   Npop_tot[17], Npop_tot[20], Npop_tot[23], Npop_tot[58], Npop_tot[60], Npop_tot[64], Npop_tot[66], Npop_tot[70], Npop_tot[72], Npop_tot[131], Npop_tot[133], Npop_tot[137],Npop_tot[139], Npop_tot[143], Npop_tot[145])/sum(Npop_tot)
  nfoi2 <- sum(Npop_tot[3],   Npop_tot[14], Npop_tot[21], Npop_tot[24], Npop_tot[52], Npop_tot[54], Npop_tot[62], Npop_tot[67], Npop_tot[68], Npop_tot[73], Npop_tot[125], Npop_tot[127], Npop_tot[135],Npop_tot[138], Npop_tot[141], Npop_tot[144])/sum(Npop_tot)
  nfoi3 <- sum(Npop_tot[4],   Npop_tot[15], Npop_tot[18], Npop_tot[25], Npop_tot[50], Npop_tot[55], Npop_tot[56], Npop_tot[61], Npop_tot[69], Npop_tot[71], Npop_tot[123], Npop_tot[126], Npop_tot[129],Npop_tot[132], Npop_tot[140], Npop_tot[142])/sum(Npop_tot)
  nfoi4 <- sum(Npop_tot[5],   Npop_tot[16], Npop_tot[19], Npop_tot[22], Npop_tot[51], Npop_tot[53], Npop_tot[57], Npop_tot[59], Npop_tot[63], Npop_tot[65],  Npop_tot[122],Npop_tot[124], Npop_tot[128],Npop_tot[130], Npop_tot[134], Npop_tot[136])/sum(Npop_tot)
  
  #need some sort of record of whether there have been previous infections with any of serotypes
  #meaning that subsequent infections could be added on top of these.
  
  # and now, we multiply the background probability of infection by the probability of 
  # encountering an infected in this serotype class
  
  #we want the background to be what actually influences this
  nfoi1[nfoi1>0]  <- 1
  nfoi2[nfoi2>0]  <- 1
  nfoi3[nfoi3>0]  <- 1
  nfoi4[nfoi4>0]  <- 1
  ...
  #sum number of infected individuals per epi class across all ages 
  #this is a list of all the epi classes across all the age classes
  #you want all the infected classes
  
  #foi is the rate at which susceptibles become infected and depends on the 
  #number of infecteds in the population at any given time - need to here multiply by the 
  #age-structured infecteds in the pop
  ...
  for (j in 1:nage) {
    ...
    #always multiply foi by the infected age structure in the pop
    mat1[1,1] <- 1- (foi[j]*nfoi1 + foi[j]*nfoi2 + foi[j]*nfoi3 + foi[j]*nfoi4)
    mat1[2,1] <- foi[j]*nfoi1 
    mat1[3,1] <- foi[j]*nfoi2 
    mat1[4,1] <- foi[j]*nfoi3
    mat1[5,1] <- foi[j]*nfoi4
    ...
    mat1[10,10] <- 1-(foi[j]*pre1*nfoi2 + foi[j]*pre1*nfoi3 + foi[j]*pre1*nfoi4)
    mat1[14,10] <- foi[j]*pre1*nfoi2 #I12
    mat1[15,10] <- foi[j]*pre1*nfoi3 #I13
    mat1[16,10] <- foi[j]*pre1*nfoi4 #I14
    
    #secondary infections - those previously infected with I2
    mat1[11,11] <- 1-(foi[j]*pre2*nfoi1 + foi[j]*pre2*nfoi3 + foi[j]*pre2*nfoi4)
    mat1[17,11] <- foi[j]*pre2*nfoi1 #I21
    mat1[18,11] <- foi[j]*pre2*nfoi3 #I23
    mat1[19,11] <- foi[j]*pre2*nfoi4 #I24
    
    #secondary infections - those previously infected with I3
    mat1[12,12] <-  1-(foi[j]*pre3*nfoi1 + foi[j]*pre3*nfoi2 + foi[j]*pre3*nfoi4)
    mat1[20,12] <- foi[j]*pre3*nfoi1 #I31
    mat1[21,12] <- foi[j]*pre3*nfoi2 #I32
    mat1[22,12] <- foi[j]*pre3*nfoi4 #I34
    
    #secondary infections - those previously infected with I4
    mat1[13,13] <-  1-(foi[j]*pre4*nfoi1 + foi[j]*pre4*nfoi2 + foi[j]*pre4*nfoi3)
    mat1[23,13] <- foi[j]*pre4*nfoi1 #I41
    mat1[24,13] <- foi[j]*pre4*nfoi2 #I42
    mat1[25,13] <- foi[j]*pre4*nfoi3 #I43
    ...
    mat1[38,38] <- 1-(foi[j]*pre1*pre2*nfoi3 + foi[j]*pre1*pre2*nfoi4)
    mat1[50,38] <- foi[j]*pre1*pre2*nfoi3
    mat1[51,38] <- foi[j]*pre1*pre2*nfoi4
    
    #tertiary infections - those previously infected with I13 (I132 and I34)
    mat1[39,39] <- 1-(foi[j]*pre1*pre3*nfoi2 + foi[j]*pre1*pre3*nfoi4)
    mat1[52,39] <- foi[j]*pre1*pre3*nfoi2
    mat1[53,39] <- foi[j]*pre1*pre3*nfoi4
    
    
    #tertiary infections - those previously infected with I14 (I142 and I43)
    mat1[40,40] <- 1-(foi[j]*pre1*pre4*nfoi2 + foi[j]*pre1*pre4*nfoi3)
    mat1[54,40] <- foi[j]*pre1*pre4*nfoi2
    mat1[55,40] <- foi[j]*pre1*pre4*nfoi3
    
    #tertiary infections - those previously infected with I21 (I213 + I214)
    mat1[41,41] <- 1-(foi[j]*pre1*pre2*nfoi3 + foi[j]*pre1*pre2*nfoi4)
    mat1[56,41] <- foi[j]*pre1*pre2*nfoi3
    mat1[57,41] <- foi[j]*pre1*pre2*nfoi4
    
    #tertiary infections - those previously infected with I23 (I231 + I234)
    mat1[42,42] <- 1-(foi[j]*pre2*pre3*nfoi1 + foi[j]*pre2*pre3*nfoi4)
    mat1[58,42] <- foi[j]*pre2*pre3*nfoi1
    mat1[59,42] <- foi[j]*pre2*pre3*nfoi4
    
    
    #tertiary infections - those previously infected with I24 (I241 + I243)
    mat1[43,43] <- 1-(foi[j]*pre2*pre4*nfoi1 + foi[j]*pre2*pre4*nfoi3)
    mat1[60,43] <- foi[j]*pre2*pre4*nfoi1
    mat1[61,43] <- foi[j]*pre2*pre4*nfoi3
    
    
    #tertiary infections - those previously infected with I31 (I312+I314)
    mat1[44,44] <- 1-(foi[j]*pre3*pre1*nfoi2 + foi[j]*pre3*pre1*nfoi4)
    mat1[62,44] <- foi[j]*pre3*pre1*nfoi2
    mat1[63,44] <- foi[j]*pre3*pre1*nfoi4
    
    #tertiary infections - those previously infected with I32 (I321+I324)
    mat1[45,45] <- 1-(foi[j]*pre3*pre2*nfoi1 + foi[j]*pre3*pre2*nfoi4)
    mat1[64,45] <- foi[j]*pre3*pre2*nfoi1
    mat1[65,45] <- foi[j]*pre3*pre2*nfoi4
    
    #tertiary infections - those previously infected with I34 (I341+I342)
    mat1[46,46] <- 1-(foi[j]*pre3*pre4*nfoi1 + foi[j]*pre3*pre4*nfoi2)
    mat1[66,46] <- foi[j]*pre3*pre4*nfoi1
    mat1[67,46] <- foi[j]*pre3*pre4*nfoi2
    
    
    #tertiary infections - those previously infected with I41 (I412+I413)
    mat1[47,47] <- 1-(foi[j]*pre4*pre1*nfoi2 + foi[j]*pre4*pre1*nfoi3)
    mat1[68,47] <- foi[j]*pre4*pre1*nfoi2
    mat1[69,47] <- foi[j]*pre4*pre1*nfoi3
    
    #tertiary infections - those previously infected with I42 (I421+I423)
    mat1[48,48] <- 1-(foi[j]*pre4*pre2*nfoi1 + foi[j]*pre4*pre2*nfoi3)
    mat1[70,48] <- foi[j]*pre4*pre2*nfoi1
    mat1[71,48] <- foi[j]*pre4*pre2*nfoi3
    
    #tertiary infections - those previously infected with I43 (I431+I432)
    mat1[49,49] <- 1-(foi[j]*pre4*pre3*nfoi1 + foi[j]*pre4*pre3*nfoi2)
    mat1[72,49] <- foi[j]*pre4*pre3*nfoi1
    mat1[73,49] <- foi[j]*pre4*pre3*nfoi2
    ...
    mat1[98, 98] <- 1-foi[j]*pre1*pre2*pre3*nfoi4
    mat1[122,98] <- foi[j]*pre1*pre2*pre3*nfoi4
    
    #quaternary infections -  S124 to I1243
    mat1[99, 99] <- 1-foi[j]*pre1*pre2*pre4*nfoi3
    mat1[123,99] <- foi[j]*pre1*pre2*pre4*nfoi3
    
    #quaternary infections -  S132 to I1324
    mat1[100, 100] <- 1-foi[j]*pre1*pre2*pre3*nfoi4
    mat1[124,100] <- foi[j]*pre1*pre2*pre3*nfoi4
    
    #quaternary infections -  S134 to I1342
    mat1[101, 101] <- 1-foi[j]*pre1*pre3*pre4*nfoi2
    mat1[125,101] <- foi[j]*pre1*pre3*pre4*nfoi2
    
    #quaternary infections -  S142
    mat1[102, 102] <- 1-foi[j]*pre1*pre4*pre2*nfoi3
    mat1[126,102] <- foi[j]*pre1*pre4*pre2*nfoi3
    
    #quaternary infections -  S143
    mat1[103, 103] <- 1-foi[j]*pre1*pre4*pre3*nfoi2
    mat1[127,103] <- foi[j]*pre1*pre4*pre3*nfoi2
    
    
    
    
    #quaternary infections -  S213
    mat1[104, 104] <- 1-foi[j]*pre2*pre1*pre3*nfoi4
    mat1[128,104] <- foi[j]*pre2*pre1*pre3*nfoi4
    
    #quaternary infections -  S214
    mat1[105, 105] <- 1-foi[j]*pre2*pre1*pre4*nfoi3
    mat1[129,105] <- foi[j]*pre2*pre1*pre4*nfoi3
    
    #quaternary infections -  S231
    mat1[106, 106] <- 1-foi[j]*pre2*pre3*pre1*nfoi4
    mat1[130,106] <- foi[j]*pre2*pre3*pre1*nfoi4
    
    #quaternary infections -  S234
    mat1[107, 107] <- 1-foi[j]*pre2*pre3*pre4*nfoi1
    mat1[131,107] <- foi[j]*pre2*pre3*pre4*nfoi1
    
    #quaternary infections -  S241
    mat1[108, 108] <- 1-foi[j]*pre2*pre4*pre1*nfoi3
    mat1[132,108] <- foi[j]*pre2*pre4*pre1*nfoi3
    
    #quaternary infections -  S243
    mat1[109, 109] <- 1-foi[j]*pre2*pre4*pre3*nfoi1
    mat1[133,109] <- foi[j]*pre2*pre4*pre3*nfoi1
    
    
    
    
    #quaternary infections -  S312
    mat1[110, 110] <- 1-foi[j]*pre3*pre1*pre2*nfoi4
    mat1[134,110] <- foi[j]*pre3*pre1*pre2*nfoi4
    
    #quaternary infections -  S314
    mat1[111, 111] <- 1-foi[j]*pre3*pre1*pre4*nfoi2
    mat1[135,111] <- foi[j]*pre3*pre1*pre4*nfoi2
    
    #quaternary infections -  S321
    mat1[112, 112] <- 1-foi[j]*pre3*pre2*pre1*nfoi4
    mat1[136,112] <- foi[j]*pre3*pre2*pre1*nfoi4
    
    #quaternary infections -  S324
    mat1[113, 113] <- 1-foi[j]*pre3*pre2*pre4*nfoi1
    mat1[137, 113] <- foi[j]*pre3*pre2*pre4*nfoi1
    
    #quaternary infections -  S341
    mat1[114, 114] <- 1-foi[j]*pre3*pre4*pre1*nfoi2
    mat1[138, 114] <- foi[j]*pre3*pre4*pre1*nfoi2
    
    #quaternary infections -  S342
    mat1[115, 115] <- 1-foi[j]*pre3*pre4*pre2*nfoi1
    mat1[139,115] <- foi[j]*pre3*pre4*pre2*nfoi1
    
    
    
    
    #quaternary infections -  S412
    mat1[116, 116] <- 1-foi[j]*pre4*pre1*pre2*nfoi3
    mat1[140,116] <- foi[j]*pre4*pre1*pre2*nfoi3
    
    #quaternary infections -  S413
    mat1[117, 117] <- 1-foi[j]*pre4*pre1*pre3*nfoi2
    mat1[141,117] <- foi[j]*pre4*pre1*pre3*nfoi2
    
    #quaternary infections -  S421
    mat1[118, 118] <- 1-foi[j]*pre4*pre2*pre1*nfoi3
    mat1[142,118] <- foi[j]*pre4*pre2*pre1*nfoi3
    
    #quaternary infections -  S423
    mat1[119, 119] <- 1-foi[j]*pre4*pre2*pre3*nfoi1
    mat1[143, 119] <- foi[j]*pre4*pre2*pre3*nfoi1
    
    #quaternary infections -  S431
    mat1[120, 120] <- 1-foi[j]*pre4*pre3*pre1*nfoi2
    mat1[144, 120] <- foi[j]*pre4*pre3*pre1*nfoi2
    
    #quaternary infections -  S432
    mat1[121, 121] <- 1-foi[j]*pre4*pre3*pre2*nfoi1
    mat1[145, 121] <- foi[j]*pre4*pre3*pre2*nfoi1
...
buildTMat_four_strain_after <- function(c, Npop, age.classes, surv.biwk, wane_hetero, age.brk, foi, age.mult.foi, recov, age.rate, pre.vect){
  
  #remove foi for serotype 1 - allow for introduction of serotype 4
  s <- nage <- length(age.classes)
...
sim.SIR.age.three.clim.wane.intro <- function(yrs, ntyr, age.brk, s,foi, age.mult.df, recov, mort, year.end, rate_hetero, clim.vect, births, pop_vector, yr.intro, biwk.intro){
  ...
  #structure the population for 4 strains even though we only have 3 to start. they will get added later
  ...
  I4_init = rep(0, s); #I4_init[1] = 0 #none to start
  I3_init = rep(0, s); I3_init[1] = 5 #comment out if you just want to check demography
  I2_init = rep(0, s); I2_init[1] = 5 #comment out if you just want to check demography
  I1_init = rep(0, s); I1_init[1] = 5 #comment out if you just want to check demography
  ...
  # foi gets spread across the biweeks as well
  # (it comes in as 1 value per year which you convert here to a biweekly
  ...
  #length(mort.biweek)!=(length(times)-1) |
  if( length(hetero.biweek)!=(length(times)-1) |length(birth_vector_biwk)!=(length(times)-1)| length(foi.biweek)!=(length(times)-1)){
    print("input vectors of unequal length")
  }
  #and your aging probability to biweeks (aging probability per year is 100%)
  #age.biwk = rep(1-exp(-(rep(1, yrs)/ntyr)), each=26)
  ...
  for (i in 2:(length(times))){
    ...
    if((times[i]<((yr.intro) + biwk.intro/26))){ 
      ...
      #each column is a timestep and each row is state, so you can see if there has ever been
      #previous primary infection with any of the four serotypes (in any age class)
      
      #calculate that here:
      
      pre1 <- sum(N_pop_ts[(c*(1:s-1) + 2),1:(i-1)], na.rm=T)
      pre2 <- sum(N_pop_ts[(c*(1:s-1) + 3),1:(i-1)], na.rm=T)
      pre3 <- sum(N_pop_ts[(c*(1:s-1) + 4),1:(i-1)], na.rm=T)
      pre4 <- sum(N_pop_ts[(c*(1:s-1) + 5),1:(i-1)], na.rm=T)
      ...
      pre.vect <- c(pre1, pre2, pre3, pre4)
      
      #then, let her go from there!
      
      Tmat <- buildTMat_four_strain(c=c, Npop= N_pop_ts[,(i-1)], age.classes=1:s, age.brk=age.brk, surv.biwk = (1-mort.df$deaths_per_cap_per_biwk),	foi = (foi.biweek[[(i-1)]]*clim.mod), age.mult.foi =age.mult.foi,  recov=recov, wane_hetero=hetero.biweek[[i-1]], age.rate=(1-exp(-(1)/ntyr)), pre.vect=pre.vect)
      
      #Tmat is infection and survival.
      transMat <- Tmat  
      
      # move forward in time - the foi here is guaranteed because the model is 
      # already fitted to the data. it does not depend on the number of 
      # susceptibles or infecteds in the population
      ...
      #introduce new infecteds into the fourth serotype (genotype)
      
      N_pop_ts[5, (i-1)] <- 5 #I4 -- here at the lowest age class
      ...
      #then, let her go from there! all strains represented
      
      Tmat <- buildTMat_four_strain(c=c, Npop= N_pop_ts[,(i-1)], age.classes=1:s, age.brk=age.brk, surv.biwk = (1-mort.df$deaths_per_cap_per_biwk),	foi = (foi.biweek[[(i-1)]]*clim.mod), age.mult.foi =age.mult.foi,  recov=recov, wane_hetero=hetero.biweek[[i-1]], age.rate=(1-exp(-(1)/ntyr)), pre.vect=pre.vect)
      #sigma=sigma.biweek[[(i-1)]], 
      #Tmat is infection and survival.
      transMat <- Tmat  
      ...
      #then, let her go from there! - this has no foi for serotype 1
      
      Tmat <- buildTMat_four_strain_after(c=c, Npop= N_pop_ts[,(i-1)], age.classes=1:s, age.brk=age.brk, surv.biwk = (1-mort.df$deaths_per_cap_per_biwk),	foi = (foi.biweek[[(i-1)]]*clim.mod), age.mult.foi =age.mult.foi,  recov=recov, wane_hetero=hetero.biweek[[i-1]], age.rate=(1-exp(-(1)/ntyr)), pre.vect = pre.vect)
...
# H0
out.cam.geno.rep.2007.uci = sim.SIR.age.three.clim.wane.intro(yrs=40,
                                              ntyr=26,
                                              s=101, 
                                              foi=c(fit.dat$uci[fit.dat$provname=="National"]), 
...
save(out.cam.geno.rep.2007.uci, file = "cam-sim-geno-rep-2007-uci.Rdata")
# 
# # 
# # test.nowane = sim.SIR.age.three.clim.wane.intro(yrs=10,
# #                                                 ntyr=26,
# #                                                 s=6, 
# #                                                 foi=c(fit.dat$lambda[fit.dat$provname=="National"])[1:10], 
# #                                                 births =  birth.dat$value[1:10], # these are per 1000
...
# #                                                 year.end = 2021)
# # 
# # #save(out.cam.geno.rep.2007, file = "cam-sim-geno-rep-2007.Rdata")
# # 
# # 
# # 
# # #and plot
...
# }
# 
# out.age <- summarise.age.dist(dat = out.cam.geno.rep.2007, year.start = min(out.cam.geno.rep.2007$year))
# 
#   
# #and plot cases
...
# }
# 
# plot.age.dist(df=out.age,slim.quant = 0.05, view.plot = T, save.plot=T, filename = "sim_intro_2007.png")
