...
# Fig3Aa <- ggplot(cam.merge) +
#   geom_sf(aes(fill=mean_age), linewidth=.6) +
#   theme_bw() +  
...
# 
# 
# Fig3Ab <- ggplot(cam.merge) +
#   geom_sf(aes(fill=slope_age), linewidth=.6) +
#   theme_bw() +  
#   theme(panel.grid = element_blank(),
#         panel.background = element_blank(),
#         axis.text = element_blank(), axis.ticks = element_blank(),
...
#                       trans="log10",
#                       labels=scales::parse_format(),
#                       name = "slope of increasing age\nin dengue infection, 2002-2020")
...
#load(paste0(homewd,"/figure-development/Fig3/comp-fits/age-mult-refit.Rdata"))#this is the age modification
load(paste0(homewd,"/figure-development/Fig3/age-fit-alt-many-short/fit-many-age-mult.Rdata"))#this is the age modification
head(fit.dat)
head(age.fit)
...
Fig3Cb <- ggplot(subset(fit.dat, provname!="Tboung Khmum")) + 
...
Fig3Cab <- cowplot::plot_grid(Fig3Ca, Fig3Cb, nrow=2, ncol = 1, rel_heights = c(.2,1))
...
age.long <- melt(age.fit, id.vars = c("age_mult", "lci_mult", "uci_mult", ), measure.vars = c("age_min", "age_max"))
head(age.long)
names(age.long)[names(age.long)=="value"] <- "age"

Fig3Cc <- ggplot(age.long) + theme_bw() + 
...
Fig3C <- Fig3Cab + annotation_custom(ggplotGrob(Fig3Cc), 
                                     xmin = .6, ymin = .52, xmax = .98, ymax = .82)
...
model.age.incidence.series.wane.age <- function(par.dat, age_vect, year.start, age_mult){
...
run.model.data.all <- function(dat,par.dat, sigma.fit, age.mult.df){
  ...
  out.mod.list <- mapply(model.age.incidence.series.wane.age, par.dat= par.dat.split, year.start=year.start.split, MoreArgs = list(age_mult = age.mult.df$age_mult, age_vect=age_vect), SIMPLIFY = FALSE)
...
out.plot <- run.model.data.all(par.dat = fit.dat, dat = dat, age.mult.df = age.mult, sigma.fit = sigma.fit)
head(out.plot)
out.plot$year <- as.factor(out.plot$year)
linez = c('data'=2 , 'model'=1)
...
Fig3D <- ggplot(subset(out.plot, provname=="National")) +
  geom_line(aes(x=age, y=cum_prop_cases, linetype=type, color=year), alpha=.8, size=.8) +
  scale_linetype_manual(values=linez) +
  ...
  theme(panel.grid = element_blank(),
        ...
        legend.position = c(.85,.46), #legend.direction = "horizontal",
        legend.title = element_blank(), strip.background = element_rect(fill="white")) +
  guides(linetype="none", color=guide_legend(ncol=1)) + 
  scale_color_viridis_d(direction = -1, option = "turbo")


# and pair with Fig3C

#Fig3CD <- cowplot::plot_grid(Fig3C, Fig3D, ncol=2, nrow=1, labels = c("C", "D"), label_size = 22)

Fig3AC <- cowplot::plot_grid(Fig3A, Fig3C, ncol=1, nrow=2, labels = c("A", "C"), label_size = 22, rel_heights = c(.43,1), label_y = c(1,.85))
Fig3BD <- cowplot::plot_grid(Fig3B, Fig3D, ncol=1, nrow=2, labels = c("B", "D"), label_size = 22, rel_heights = c(.7,1), align = "hv")


# and all together
#Fig3 <- cowplot::plot_grid(Fig3AB, Fig3CD, ncol=1, nrow = 2, rel_heights = c(.7,1)) + theme(plot.background = element_rect(fill="white"))
Fig3 <- cowplot::plot_grid(Fig3AC, Fig3BD, ncol=2, nrow = 1) + theme(plot.background = element_rect(fill="white"))
