...
setwd(paste0(homewd, "/figure-development/Fig3/cohort-fit/"))
  ...
  for (i in 1:lts){
    ...
    for(a in 2:(length(age_vect_tmp))){ 
      ...
      age.mult.df <- cbind.data.frame(mult=age_mult, 
                                      ...
                                      age_max=c(2,4,6,9,12,15,19,29,39,100))
...
log.lik.fit.all.prov.age <- function(par, par.dat, dat){
  ...
  out.mod.list <- mapply(model.age.incidence.series.age, par.dat= par.dat.split, year.start=year.start.split, MoreArgs = list(age_mult=age.mult, age_vect=age_vect), SIMPLIFY = FALSE)
...
log.lik.fit.two.ts.age <- function(par, par.dat, age_vect, dat){
  ...
  out.mod <- model.age.incidence.series.age(par.dat = par.dat, year.start = min(par.dat$year), age_mult = age.mult, age_vect=age_vect)  
  ...
  out.merge <- dplyr::select(out.mod, year, age, N_sero, all_prim, cum_prop_cases)
  names(out.merge)[names(out.merge)=="cum_prop_cases"] <- "cum_prop_cases_model"
  dat <- arrange(dat, year, age)
  
  merge.dat <- merge(dat, out.merge, by=c("year", "age"), all.x = T, sort = F)
  
  #merge data and model and fit to both primary and secondary cases
  ...
  for (i in 1:length(merge.dat$age)){
    ll=ll+dbinom(merge.dat$cum_cases[i],merge.dat$n_tot_multi[i],p=merge.dat$cum_prop_cases_model[i],log=T) 
    ll=ll+dbinom(merge.dat$n_prim_cases[i],merge.dat$n_tot_prim[i],p=merge.dat$all_prim[i],log=T) #and prim
  }
...
fit.age.mult.LBFGSB.all <- function(dat,par.dat, fit.CI){
  ...
  #first, prep the data - split by year
  year.dat <- dlply(dat, .(year))
  
  
  age_vect=seq(0, max(dat$age[dat$year==min(dat$year)]), by=1)
  
  
  #age_vect_year = floor(age_vect)[!duplicated(floor(age_vect))]
  df.out = data.table::rbindlist(lapply(year.dat, sum.dat.igg, age_vect=age_vect))
  
  
  
  # we already have par.dat
 
  #now take a guess at 10 age-specific multiplicative factors for ages:
  #1-2,3-4,5-6,7-9,10-12,13-15,16-19,20-29,30-39,40+
  age.guess <- rep(1,10)
...
#dat <- read.csv(file = "IDseq_PAGODAS_ALL_metadata_through_2020_CLEAN.csv", header=T, stringsAsFactors = F)
...
dat.pos$igg_res[dat.pos$igg_res=="#N/A"] <- NA
dat.pos$igm_res[dat.pos$igm_res=="#N/A"] <- NA
dat.pos$combined_elisa = paste0("igm-",dat.pos$igm_res,"-igg-",dat.pos$igg_res)
unique(dat.pos$combined_elisa)
dat.pos$combined_elisa <- factor(dat.pos$combined_elisa, levels=c("igm-NA-igg-NA", "igm-NA-igg-neg", "igm-neg-igg-neg", "igm-pos-igg-neg", "igm-pos-igg-pos", "igm-neg-igg-pos"    ))
ggplot(subset(dat.pos, combined_elisa!="igm-NA-igg-NA" & DENV.serotype!="DENV-4" & !is.na(igm_res))) + 
  geom_boxplot(aes(x=combined_elisa, y=age, color=temp)) +facet_grid(~DENV.serotype) +theme(axis.title.x = element_blank(), axis.text.x = element_text(angle=90))


ggplot(subset(dat.pos, !is.na(igg_res) &DENV.serotype!="DENV-4"))+ geom_boxplot(aes(x=igg_res, y=age)) +facet_grid(~DENV.serotype)
ggplot(subset(dat.pos, !is.na(igm_res) &DENV.serotype!="DENV-4"))+ geom_boxplot(aes(x=igm_res, y=age)) +facet_grid(~DENV.serotype)


ggplot(subset(dat.pos, !is.na(igg_res)))+ geom_jitter(aes(x=igg_res, y=age, color=temp)) +facet_grid(~DENV.serotype)
ggplot(subset(dat.pos, !is.na(igm_res)))+ geom_jitter(aes(x=igm_res, y=age, color=temp)) +facet_grid(~DENV.serotype)
...
#load the lambda fits
load(paste0(homewd,"/figure-development/Fig3/cohort-fit/foi-kampong-speu-2019-cohort-refit.Rdata"))


out <- fit.age.mult.LBFGSB.all(dat=dat.pos,
                          par.dat=fit.KP.2019,
                          fit.CI = TRUE)


save(out, file = "fit-age-cohort.Rdata")

age.long <- melt(out, id.vars = c("age_mult", "lci_mult", "uci_mult"), measure.vars = c("age_min", "age_max"))

head(age.long)
names(age.long)[names(age.long)=="value"] <- "age"

pPlot <- ggplot(age.long) + theme_bw() +
  geom_line(aes(x=age, y=age_mult), size=1) +
...
ggsave(filename = "age-multiplier-cohort.png",
       plot = pPlot,
       units="mm",  
       width=55, 
       height=40, 
...
#compare with and without age data
compare.llik.LRT.cohort <- function(par.dat, dat,  age.mod){
  ...
  llik <- log.lik.fit.two.ts.age(par=age.mod$age_mult, par.dat = par.dat, dat=df.out, age_vect = age_vect)
  
  par.dat$llik_w_age <- llik
  
  #now attach and compare with rest of par.dat
  par.dat$LRT_stat <- LRTest(full=(-par.dat$llik_w_age[1]), reduced = (-par.dat$llik[1]), df=10, boundaryCorrection = F)$lambda
  par.dat$LRT_pval <- LRTest(full=(-par.dat$llik_w_age[1]), reduced = (-par.dat$llik[1]), df=10, boundaryCorrection = F)$Pval
...
out.comp <- compare.llik.LRT(par.dat= fit.KP.2019,  
                             dat=dat.pos, 
                             age.mod=out)
