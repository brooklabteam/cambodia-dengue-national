...
#setwd(paste0(homewd, "/figure-development/Fig3/age-fit-alt-two-short/age-up"))
...
log.lik.fit.all.prov.age.NM <- function(par, par.dat, dat){
...
refit.age.mult.NM <- function(dat,par.dat, age.mult, fit.CI){
  ...
  age.guess <- age.mult$age_mult
  
  #lambda.guess <- par.dat$lambda
  
  
  out.NS <- optim(par = log(age.guess), 
                  fn=log.lik.fit.all.prov.age.NM, 
                  method = "Nelder-Mead",
  ...
  age.mult.df <- cbind.data.frame(age_mult=exp(c(out.NS$par)), 
                                  age_min=rep(c(1,3,5,7,10,13,16,20,30,40,50,60,70),2),
                                  age_max=rep(c(2,4,6,9,12,15,19,29,39,49,59,69,90),2),
                                  year_range = c(rep("2002-2010",13), rep("2011-2020", 13)))
...
age.refit.3 <- refit.age.mult.NM(dat=dat,
                          par.dat=fit.dat,
                          age.mult=age.fit,
                          fit.CI = TRUE)


save(age.refit.3, file = "refit-many-age-mult-3.Rdata")
