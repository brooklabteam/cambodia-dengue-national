...
setwd(paste0(homewd, "/figure-development/Fig3/"))
...
log.lik.fit.all.prov.age.profile <- function(par, num, age.mult.df, par.dat, dat){
  
  #par.dat$lambda does not change here
  age.mult <- age.mult.df$age_mult
  age.mult[num] <- par
...
get.age.profile <- function(num1, age.mult.df, par.dat, dat1){
  age.guess = seq(0.0000001, 5, length=100)
  
  
  llik <- list()
  for(i in 1:length(age.guess)){
    llik[[i]] <- log.lik.fit.all.prov.age.profile(par = age.guess[i], num = num1, age.mult.df = age.mult.df, par.dat = par.dat, dat=dat1)  
  }
  
  out.lik <- cbind.data.frame(par = age.guess, llik=c(unlist(llik)))
  ...
  age.mult$lci_mult[num1] <- conf.int[1]
  age.mult$uci_mult[num1] <- conf.int[2]
  
  age.sub = subset(age.mult, lci_mult==conf.int[1])
  
  return(age.sub)
}
profile.age.fit <- function(dat,par.dat, age.mult, fit.CI){
  ...
  #now get age multipliers across all 13 age classes
  profile.out <- lapply(as.list(1:26), get.age.profile, age.mult.df =age.mult, par.dat=par.dat, dat1=df.out)
  
  
  profile.df <- data.table::rbindlist(profile.out)
  #and return
...
load(paste0(homewd,"/figure-development/Fig3/age-fit-alt-two-short/age-up/refit-many-age-mult-2.Rdata"))#this is the age modification
fit.dat <- read.csv(file = paste0(homewd,"/data/prov-fits-FOI.csv"), header = T, stringsAsFactors = F)

profile.df <- profile.age.fit(dat=dat,
                          par.dat=fit.dat,
                          age.mult=age.refit.2,
                          fit.CI = TRUE)


save(profile.df, file = "age-mult-profile.Rdata")
