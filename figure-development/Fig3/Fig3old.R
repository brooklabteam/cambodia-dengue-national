...
# Now add in the FOI
load(paste0(homewd,"/figure-development/Fig3/age-fit-alt-3/prov-fits-FOI.Rdata")) #these are the fitted pars
load(paste0(homewd,"/figure-development/Fig3/age-fit-alt-3/fit-all-provinces-age-mult.Rdata"))#this is the age modification
head(fit.dat)
head(out)
...
run.model.data.all <- function(par.dat, dat,  age.mod){
  ...
  out.mod.list <- mapply(model.age.incidence.series.age, par.dat= par.dat.split, year.start=year.start.split, MoreArgs = list(age_mult = age.mod$age_mult, age_vect=age_vect), SIMPLIFY = FALSE)
...
out.plot <- run.model.data.all(par.dat = fit.dat, dat = dat, age.mod=out[1:10,])
head(out.plot)
