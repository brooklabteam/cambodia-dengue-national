...
log.lik.wane.age.fit.all <- function(age.mult, par.dat, dat){
  ...
  out.mod <-model.age.incidence.series.age.mult.wane(par.dat= par.dat, year.start=year.start, age_mult=age.mult, age_vect=age_vect)
  ...
  #  ggplot(data=out.merge) + facet_wrap(~year) +  geom_point(aes(x=age, y=cum_prop_cases_data)) + geom_line(aes(x=age, y=cum_prop_cases_data), size=1) +    geom_line(aes(x=age,y= cum_prop_cases), color="tomato") 
  # # 
...
log.lik.fit.age <- function(par.dat, age.mult, dat){
  ...
  out.mod <- model.age.incidence.series.age.mult.wane(par.dat= par.dat, year.start=min(par.dat$year), age_mult=age.mult$age_mult, age_vect=age_vect)
...
run.comp.models <- function(dat,par.dat, sigma.fit, age.mult.df){
  ...
  #now get it for age-fit only
  par.dat$llik_age <- log.lik.fit.age(par.dat = par.dat, age.mult=age.mult.df, dat=df.out)
  ...
  par.dat$llik_sigma_age = log.lik.wane.age.fit.all(age.mult = age.mult.df$age_mult, par.dat = par.dat, dat=df.out)
  
  par.slim <- dplyr::select(par.dat, llik, llik_age, llik_sigma, llik_sigma_age, provname)
  ...
  par.slim$AIC_age <- (2*(nrow(par.dat)+nrow(age.mult.df)))+(2*par.slim$llik_age)
  par.slim$AIC_sigma <- (2*(nrow(par.dat)+nrow(sigma.fit)))+(2*par.slim$llik_sigma)
  par.slim$AIC_sigma_age <- (2*(nrow(par.dat)+nrow(sigma.fit)+nrow(age.mult.df)))+(2*par.slim$llik_sigma_age)
...
comp.split <- mapply(run.comp.models, dat=dat.split, par.dat=fit.split, MoreArgs = list(sigma.fit = sigma.fit, age.mult.df = age.fit), SIMPLIFY = F)
#comp.split <- mapply(run.comp.models.10, dat=dat.split, par.dat=fit.split, MoreArgs = list(sigma.fit = sigma.fit, age.mult.df = out), SIMPLIFY = F)

comp.df <- data.table::rbindlist(comp.split)

#save this
TableS8B <- dplyr::select(comp.df, provname, llik_FOI, llik_age, llik_sigma, llik_sigma_age, AIC_FOI, AIC_age, AIC_sigma, AIC_sigma_age)
write.csv(TableS8B, file = paste0(homewd,"/data/TableS8_part2.csv"), row.names = F)


comp.df.long <- melt(comp.df, id.vars = c("provname"))
comp.df.long$variable_type <- sapply(strsplit(as.character(comp.df.long$variable), "_"),'[',1)
comp.df.AIC = subset(comp.df.long, variable_type=="AIC")
comp.df.AIC <- arrange(comp.df.AIC, provname, variable)
AIC.split<- dlply(comp.df.AIC,.(provname))
get.min <- function(df){
  df1 <- df[df$value==min(df$value),]
  return(df1)
}
AIC.winner <- data.table::rbindlist(lapply(AIC.split, get.min))
AIC.winner

AIC.sum = ddply(AIC.winner, .(variable), summarise,N=length(variable_type))
AIC.sum #usually sigma + age (including for National)
