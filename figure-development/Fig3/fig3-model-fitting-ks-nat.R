...
fit.one.yr.only.BFGS <- function(dat, par.dat, lambda.guess, N.sero.fix, age_vect, fit.CI){
  ...
  df.out <- sum.yr.yr(df=dat, age_vect = age_vect_year)
  ...
  if(fit.CI==TRUE){
    ...
    llik.list <- list()
      for (i in 1:length(lambda.test)){
        ...
        llik.list[[i]] <- log.lik.fit.all(par=lambda.test[i], par.dat=par.dat, age_vect=age_vect, dat=df.out)
      }
     
    dat.CI <- cbind.data.frame(lambda = exp(lambda.test), neg_llik=c(unlist(llik.list)))
    
    #with(dat.CI, plot(lambda, neg_llik, type="l"))
...
#load the fitted data from the national level and estimate just the 2019 results by fitting to the Kampong Speu subset
par.dat <- read.csv(file = paste0(homewd, "/data/foi-fit-dat.csv"), stringsAsFactors = F, header = T)
head(par.dat)

fit.KP.2019 <- fit.one.yr.only.BFGS(dat=dat,
