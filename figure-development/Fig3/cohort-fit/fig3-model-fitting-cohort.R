...
fit.two.ts.all.yrs.BFGS <- function(dat, par.dat, lambda.guess, N.sero.fix, fit.CI){
...
plot.model.series.data <- function(par.dat, dat){
  ...
  year.dat.sum <- lapply(year.dat, sum.yr.yr, age_vect = age_vect)
...
load(paste0(homewd,"figure-development/Fig3/cohort-fit/prov-fits-FOI.Rdata"))
#load("prov-fits-FOI.Rdata")
#head(fit.dat)

par.dat = subset(fit.dat, provname=="Kampong Speu")
#head(par.dat) #starts in 1988... use fits from national for the first 5 years
nat.dat = subset(fit.dat, provname=="National")
nat.add = subset(nat.dat, year>=1983 & year<1988)
par.dat <- rbind(nat.add, par.dat)
...
fit.KP.2019 <- fit.two.ts.all.yrs.BFGS(dat=dat.pos,
                                    ...
                                    fit.CI = T)
#now write data
save(fit.KP.2019, file = paste0(homewd,"figure-development/Fig3/cohort-fit/foi-kampong-speu-2019-cohort.Rdata"))
...
plot.model.series.data(par.dat = fit.KP.2019, dat=dat.pos)

#and plot with the national data
ggplot(data=fit.KP.2019) + 
      geom_line(aes(x=year, y=lambda), color="tomato") +
      geom_point(aes(x=year, y=lambda), color="tomato") +
      geom_line(data=par.dat, aes(x=year, y= lambda)) +
      geom_point(data=par.dat, aes(x=year, y= lambda)) +
      geom_line(data=subset(fit.dat, provname=="National"), aes(x=year, y= lambda), color="cornflowerblue") +
      geom_point(data=subset(fit.dat, provname=="National"), aes(x=year, y= lambda), color="cornflowerblue") 

#now send off to fit ages too