...
pB <- ggplot(data=subset(all.dat, year>=2002)) + 
      ...
      theme(panel.grid = element_blank(), axis.title.x = element_blank(),
        ...
        axis.text = element_text(size=14), plot.tag = element_text(size=22, face="bold")) + 
        scale_color_manual(values=yearz) +
       geom_line(aes(x=year, y=lambda, linetype = type)) + 
      geom_vline(aes(xintercept=2018.5), linetype=2, color="red") +
      ylab(bquote(lambda~',force of infection (per capita)')) +
      geom_point(aes(x=year, y=lambda, color=year_plot, group=type), size=3, show.legend = F) +
      geom_ribbon(aes(x=year, ymin=lci, ymax=uci, group = type), alpha=.3) +scale_linetype_manual(values=c(1,2)) +
      labs(tag = "b") 
