...
log.lik.fit.all <- function(par, par.dat, age_vect, dat, year.start){
  ...
  #and select only the data that drive the models
  names(dat)[names(dat)=="cum_prop_cases"] <- "cum_prop_cases_data"
  out.merge <- merge(out.mod, dat, by = c("year", "age"))
...
log.lik.fit.one <- function(par, par.dat, index, age_vect, dat, year.start){
  
  par.dat$lambda[index] <- exp(par)
...
fit.one.yr.BFGS <- function(dat, dist.back, index, lambda.guess, N.sero.fix, age_vect, fit.CI){
  ...
  par.dat$lambda[index] <- exp(out.NS$par)
  par.dat$llik <- out.NS$value
  par.dat$convergence <- out.NS$convergence
  ...
  if(fit.CI==TRUE){
    ...
    if (is.positive.definite(out.NS$hessian)==TRUE){
      ...
      par.dat$lci[index] <- CI[,1]
      par.dat$uci[index] <- CI[,2]
...
lambda.guess.dat <- read.csv(file = paste0(homewd, "data/foi-fit-national.csv"), header = T, stringsAsFactors = F)
...
write.csv(fit.KP.nat.all, file = paste0(homewd, "/data/foi-fit-KP-national.csv"), row.names = F)
with(subset(fit.KP.nat.all, year>=2002), plot(year, lambda, type="b"))
with(fit.KP.nat.all, plot(year, lambda, type="b"))

#now try to get better bounds on that 2019 estimate

fit.KP.nat.all <- fit.one.yr.BFGS(dat=dat, dist.back = 16,
                                          par.dat = fit.KP.nat.all,
                                          lambda.guess = 0.09,
                                          age_vect=seq(0,22, by=1/4),
                                          fit.CI = T)