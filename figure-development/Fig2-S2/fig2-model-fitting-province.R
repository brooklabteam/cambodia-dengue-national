...
fit.CI = F
 ...
 wrap.fit.province <- function(dat.all, lambda.guess, N.sero.fix, age_vect, fit.CI ){
   
#split by province
dat.split <- dlply(dat.all, .(procode))

#now apply this across each
foi.fit.province.list <- lapply(dat.split, fit.all.yrs.seq.yr.BFGS, lambda.guess=lambda.guess, N.sero.fix= N.sero.fix, age_vect=age_vect, fit.CI = fit.CI)


 }
write.csv(foi.fit.national, file = paste0(homewd, "/data/foi-fit-national.csv"), row.names = F)

with(foi.fit.national, plot(year[22:40], lambda[22:40], type="b"))
with(foi.fit.national, plot(year, lambda, type="b"))

################################################################
################################################################

#par.profile <- read.csv(file =paste0(homewd, "/data/foi-fit-national.csv"), header = T, stringsAsFactors = F)

#and add in some min/max guesses
par.profile$lambda_min <- 0.0001
par.profile$lambda_max <- 1
par.profile$lambda_max[1] <- 150
par.profile$lambda_min[3] <- 0.000000001
par.profile$lambda_min[4] <- 0.00001
...
profile.CIs
#write.csv(profile.CIs, file = paste0(homewd, "/data/foi-fit-national.csv"), row.names = F)
