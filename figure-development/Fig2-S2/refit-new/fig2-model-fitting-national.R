...
#setwd(paste0(homewd, "/figure-development/Fig2-S2/refit-new/"))
...
out <- fit.all.yrs.seq.yr.BFGS(dat=dat,
                              lambda.guess=out$lambda,
                               N.sero.fix= 2,
                               fit.CI = F)
out$provname = "national"
# 
# out1 <- out
# 
# out <- fit.all.yrs.seq.yr.BFGS(dat=dat,
#                                lambda.guess=out1$lambda,
#                                N.sero.fix= 2,
#                                fit.CI = F)
# 
# 
# ggplot(out) + geom_line(aes(x=year, y=lambda)) + 
#               geom_point(aes(x=year, y=lambda)) +
...
save(out, file = "/home/cbrook/refit-new/out/fit-national.Rdata")
