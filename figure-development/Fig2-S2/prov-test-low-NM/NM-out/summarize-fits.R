...
setwd(paste0(homewd, "/figure-development/Fig2-S2/prov-test-low-NM/NM-out/"))
...
p1 <- ggplot (fit.dat) + geom_point(aes(x=year, y=lambda, color=provname)) + 
      ...
      theme(panel.grid = element_blank(), axis.title.x = element_blank(), 
            axis.title.y = element_text(size=16), axis.text = element_text(size=14)) + coord_cartesian(ylim = c(0,.01)) +
...
ggplot (fit.dat) + geom_point(aes(x=year, y=lambda, color=provname)) +
  ...
  theme(panel.grid = element_blank()) + coord_cartesian(ylim = c(0,.01)) +
  geom_vline(xintercept = 2007, linetype=2) +
  geom_vline(xintercept = 2012, linetype=2) +
  geom_vline(xintercept = 2019, linetype=2) + facet_wrap(~provname, ncol=4)


ggplot (subset(fit.dat, provname=="Phnom Penh")) + geom_point(aes(x=year, y=lambda, color=provname)) +
  geom_line(aes(x=year, y=lambda, color=provname)) +theme_bw() + 
  theme(panel.grid = element_blank()) + coord_cartesian(ylim = c(0,.005)) +
  geom_vline(xintercept = 2007, linetype=2) +
  geom_vline(xintercept = 2012, linetype=2) +
  geom_vline(xintercept = 2019, linetype=2) + facet_wrap(~provname, ncol=4)
