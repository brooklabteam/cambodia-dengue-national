...
#setwd(paste0(homewd, "/figure-development/Fig2-S2/age-lambda/"))
...
model.age.incidence.series <- function(par.dat, age_vect, age.mod, age.adj, year.start){
  ...
  #set up age-structured lambdas
  
  age.adj.dat <- cbind.data.frame(age=age_vect)
  
  if(age.mod=="age_mult"){
    age.adj.dat$lambda_mod <- 1  
    
  }else if (age.mod=="age_add"){
    age.adj.dat$lambda_mod <- 0
  
  }
  
  #now fill in
  age.adj.dat$lambda_mod[age.adj.dat$age<1]<- age.adj[1]
  age.adj.dat$lambda_mod[age.adj.dat$age>=1 & age.adj.dat$age<2] <- age.adj[2]
  age.adj.dat$lambda_mod[age.adj.dat$age>=2 & age.adj.dat$age<3] <- age.adj[3]
  age.adj.dat$lambda_mod[age.adj.dat$age>=3 & age.adj.dat$age<4] <- age.adj[4]
  age.adj.dat$lambda_mod[age.adj.dat$age>=4 & age.adj.dat$age<5] <- age.adj[5]
  age.adj.dat$lambda_mod[age.adj.dat$age>=5 & age.adj.dat$age<6] <- age.adj[6]
  age.adj.dat$lambda_mod[age.adj.dat$age>=6 & age.adj.dat$age<7] <- age.adj[7]
  
  age.adj.dat$lambda_mod[age.adj.dat$age>=7 & age.adj.dat$age<11] <- age.adj[8]
  age.adj.dat$lambda_mod[age.adj.dat$age>=11 & age.adj.dat$age<16] <- age.adj[9]
  age.adj.dat$lambda_mod[age.adj.dat$age>=16 & age.adj.dat$age<21] <- age.adj[10]
  ...
  for (i in 1:lts){
    ...
    for(a in 2:(length(age_vect_tmp))){ #for-loops over all possible ages in our data range across all the years
      ...
      #age.modifier = age.adj.dat$lambda_mod[age.adj.dat$age<=age_current]
      #now select age.adjustment
      #we have 10 values of age modification. they will 1 each for first 6 years of life,
      #then 1 for ages 6-10, 1 for ages 11-15, and one for ages 15-20
      ...
      age.modifier = dur #if there are zeros, it does not matter since you spent no
      #time here, but for places where there are values, they must be modified by age
      
      age.modifier[age.modifier>0] <- age.adj.dat$lambda_mod[1:length(age.modifier[age.modifier>0])]
      
      
      #now, modify lambda by the age modifier
      if(age.mod=="age_mult"){
        age.modifier[age.modifier==0] <- 1 
        lambda = lambda*age.modifier
      }else if(age.mod=="age_add"){
        lambda = lambda+age.modifier
...
log.lik.fit.all <- function(par, par.dat, dat, age.mod, year.start){
  
  par.dat$lambda <- exp(par[1:nrow(par.dat)])
  
  age.adj = exp(par[(nrow(par.dat)+1):length(par)])
  
  age_vect=seq(0, max(dat$age), by=1/4)
  
  out.mod <- model.age.incidence.series(par.dat = par.dat, year.start, age.mod=age.mod, age.adj=age.adj,
                                        age_vect=age_vect)  
...
fit.all.yrs.seq.yr.BFGS <- function(dat, age.mod, lambda.guess, N.sero.fix,  fit.CI){
  ...
  #unique lambda for every year for every province, but united by shared multipliers by age
  #head(df.out)
  ...
  #max(dat$age)
  #for ages 1-20, either an additive or multiplicative age parameter is introduced
  if(age.mod=="age_mult"){
    age.adj = rep(1.5,10)
  }else if(age.mod=="age_add"){
    age.adj = rep(.5,10)  
    ...
    log.lambda.guess <- log(c(par.dat$lambda, age.adj))
...
#dat = subset(dat, provname=="Kampong Cham")
...
out <- fit.all.yrs.seq.yr.BFGS(dat=dat,
                              ...
                              lambda.guess=0.001,
                               N.sero.fix= 2,
                               fit.CI = F)
out$provname = "national-age-mult"

save(out, file = "fit-national-age-mult.Rdata")
