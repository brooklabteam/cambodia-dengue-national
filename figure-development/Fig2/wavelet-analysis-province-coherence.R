...
#once a-j that is finished, load it here and run to find:
...
prov.rank.multi <- function(df2, df1, pval.cut){
  ...
  df1a <-  df1
  df2a <- df2
  df1 <- dplyr::select(df1, time, reconstructed_multi_period )
  df2 <- dplyr::select(df2, time, reconstructed_multi_period )
  
  names(df1)[names(df1)=="reconstructed_multi_period"] <- "cases_this_prov"
  names(df2)[names(df2)=="reconstructed_multi_period"] <- "cases_other_prov"
  ...
  df.merge$coherence_multi_cycle <- colMeans(out.mat, na.rm = T)
  
  
  df2add <- dplyr::select(df.merge, time, coherence_multi_cycle)
  
  df1a <- merge(df1a, df2add, by="time", all.x = T)
  
  df1a$coherence_prov <- provname
  
  df.out <- dplyr::select(df1a, time, coherence_multi_cycle, coherence_prov)
  df.out <- arrange(df.out, time)
  #ggplot(df.out) + geom_line(aes(x=time, y=coherence_multi_cycle))
  ...
  df.out$cross_wave_power_multi_cycle <- colMeans(power.mat, na.rm=T)
  
  
  #ggplot(df.out) + geom_line(aes(x=time, y=cross_wave_power_multi_cycle))
  return(df.out)
  
  
}
prov.rank.annual.cases <- function(df2, df1, pval.cut){
  
  df1a <-  df1
  df2a <- df2
  df1 <- dplyr::select(df1, time, cases_per_100000)
  df2 <- dplyr::select(df2, time, cases_per_100000)
  
  names(df1)[names(df1)=="cases_per_100000"] <- "cases_this_prov"
  names(df2)[names(df2)=="cases_per_100000"] <- "cases_other_prov"
...
prov.rank.annual.cycle <- function(df2, df1, pval.cut){
  
  df1a <-  df1
  df2a <- df2
  df1 <- dplyr::select(df1, time, reconstructed_annual_period)
  df2 <- dplyr::select(df2, time, reconstructed_annual_period)
  
  names(df1)[names(df1)=="reconstructed_annual_period"] <- "cases_this_prov"
  names(df2)[names(df2)=="reconstructed_annual_period"] <- "cases_other_prov"
  ...
  #df.merge$coherence_annual <- colSums(pval.mat)
  df.merge$coherence_annual_cycle <- colMeans(out.mat, na.rm = T)
  
  
  df2add <- dplyr::select(df.merge, time, coherence_annual_cycle)
  
  df1a <- merge(df1a, df2add, by="time", all.x = T)
  
  df1a$coherence_prov <- provname
  
  df.out <- dplyr::select(df1a, time, coherence_annual_cycle, coherence_prov)
  df.out <- arrange(df.out, time)
  #ggplot(df.out) + geom_line(aes(x=time, y=coherence_annual_cycle))
  ...
  df.out$cross_wave_power_annual_cycle <- colMeans(power.mat, na.rm=T)
  
  
  #ggplot(df.out) + geom_line(aes(x=time, y=cross_wave_power_annual_cycle))
  return(df.out)
...
get.all.coherence <- function(dat, dat.all, pval.cut1){
  dat.all = subset(dat.all, provname !=unique(dat$provname))
  ...
  dat.prov.coherence.multi <- data.table::rbindlist(dat.prov.coherence.multi)
  dat.prov.coherence.annual.cases <- lapply(X=df.split, FUN=prov.rank.annual.cases, df1=dat,  pval.cut=pval.cut1)
  dat.prov.coherence.annual.cases <- data.table::rbindlist(dat.prov.coherence.annual.cases)
  dat.prov.coherence.annual.cycles <- lapply(X=df.split, FUN=prov.rank.annual.cycle, df1=dat,  pval.cut=pval.cut1)
  dat.prov.coherence.annual.cycles <- data.table::rbindlist(dat.prov.coherence.annual.cycles)
  #head(dat.prov.coherence.multi)
  #head(dat.prov.coherence.annual)
  
  
  #now attach to the rest of the dataset
  names(dat.prov.coherence.annual.cases) <- names(dat.prov.coherence.multi) <- names(dat.prov.coherence.annual.cycles) <- c("time", "coherence", "coherence_prov", "cross_wave_power")
  dat.prov.coherence.annual.cases$cycle_type <- "annual_cases"
  dat.prov.coherence.annual.cycles$cycle_type <- "annual_cycles"
  dat.prov.coherence.multi$cycle_type <- "multi_cycles"
  
  dat.out <- rbind(dat.prov.coherence.annual.cases, dat.prov.coherence.annual.cycles, dat.prov.coherence.multi)
  dat.out$provname <- unique(dat$provname)
  
  return(dat.out)
}

#first, load the reconstructed cycle data 
corr.dat <- read.csv(paste0(homewd, "/data/synchrony_case_data_oct15.csv"), header = T, stringsAsFactors = F)

#then, split by province
prov.split <- dlply(corr.dat, .(provname))

#then, get pairwise coherence (both by cross-wavelet power and by coherence) across all provinces
all.coherence.out <- lapply(prov.split, get.all.coherence, dat.all=corr.dat, pval.cut1=0.01)
coherence.df <- data.table::rbindlist(all.coherence.out)

write.csv(coherence.df, file = paste0(homewd, "/data/annual_multi_coherence_oct15.csv"), row.names = F)
