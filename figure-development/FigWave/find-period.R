...
plot.multi.annual.period <- function(dat){
  ...
  anal.dat.multi <-  analyze.wavelet(dat,
                                     ...
                                     lowerPeriod = 1.5,#shortest possible period (1.5)
                                     upperPeriod = 10, #here, 5 years
  ...
  #plot
  return(
  wt.image(anal.dat.multi, color.key = "quantile", n.levels = 250,
           legend.params = list(lab = "wavelet power levels", mar = 4.7),
           spec.period.axis = list(at = c(2:10,12,14,16), labels = c(2:10,12,14,16)),
           periodlab= "period (in years)", main= unique(dat$provname),
           plot.coi = F,
...
for(i in 1:25){
  png(paste0(homewd, "/wave-plot/multi_prov_", unique(prov.split[[i]]$provname),".png"))
  plot.multi.annual.period(prov.split[[i]])
  dev.off()
}
...
p.list <- list()
for (i in 1:25){
  tmp <- readPNG(paste0(homewd, "/wave-plot/multi_prov_", unique(prov.split[[i]]$provname),".png"))
  p.list[[i]] <- rasterGrob(tmp, interpolate=TRUE)
}

pall <- cowplot::plot_grid(plotlist = p.list, ncol=5, nrow=5)

ggsave(file = paste0(homewd,"/test-figs/MultiAllProvinceWaveletDetrend_10years.png"),
       plot=pall,
       units="mm",  
       width=110, 
       height=90, 
...
#and national?
nat.dat <- ddply(climdat, .(month_date), summarize, cases_per_1000=sum(cases_per_1000))

anal.dat.multi <-  analyze.wavelet(nat.dat,
                                   my.series = "cases_per_1000", #cases per 1000
                                   #loess.span = 0,
                                   #method = "Fourier.rand",
                                   dt = 1/12,#this allows for annual timestep
...
png(paste0(homewd,"/test-figs/NationalWaveletDetrend.png"))
wt.image(anal.dat.multi, color.key = "quantile", n.levels = 250,
         legend.params = list(lab = "wavelet power levels", mar = 4.7),
         spec.period.axis = list(at = c(2:10,12,14,16), labels = c(2:10,12,14,16)),
         periodlab= "period (in years)", main= "National",
         plot.coi = F,
         spec.time.axis = list(at = seq(1,12*18, 12), #converting weeks to years
                               labels = 2002:2019))
dev.off()
#   per.df = extract.period(wave.out=anal.dat.multi, dat=dat)
#   
# }
...
extract.ridges <- function(x, per, ts){
  index = which(x==1)  
  if(length(index)==1){
    per.ret = per[index]
    new.dat = cbind(period=per.ret)  
  }else if(length(index)==0){
    new.dat = cbind(period=NA)  
  }else if(length(index)>0){
    per.ret = per[index]
    new.dat = cbind.data.frame(period=per.ret)
  }
  
  
  return(new.dat)
}
extract.period <- function(wave.out, dat){
  df = wave.out$Ridge
  col.split = lapply(seq_len(ncol(df)), function(i) df[,i])
  col.ret = lapply(col.split, extract.ridges, per = wave.out$Period)
  
  period.df= cbind.data.frame(time = dat$month_date,  period= c(unlist(col.ret)))
...
get.wavelet.dat <- function(dat, dat.all){
  ...
  anal.dat.annual <-  analyze.wavelet(dat,
                                     ...
                                     dt = 1/12,#this allows for annual timestep
                                     dj = 1/100, #default =1/20. image gets clearer as number on the bottom gets bigger
                                     lowerPeriod = 1/12,#shortest possible period ()
...
anal.dat.multi <-  analyze.wavelet(dat,
                                   ...
                                   #method = "Fourier.rand",
                                   dt = 1/12,#this allows for annual timestep
                                   dj = 1/200, #default =1/20. image gets clearer as number on the bottom gets bigger
                                   lowerPeriod = 1.5,#shortest possible period (1.5)
                                   upperPeriod = 10, #here, 5 years
                                   make.pval = TRUE, n.sim = 200)
...
wt.image(anal.dat.multi, color.key = "quantile", n.levels = 250,
         legend.params = list(lab = "wavelet power levels", mar = 4.7),
         spec.period.axis = list(at = c(2:10,12,14,16), labels = c(2:10,12,14,16)),
         periodlab= "period (in years)", main= "Banteay",
         plot.coi = F,
         spec.time.axis = list(at = seq(1,12*18, 12), #converting weeks to years
                               labels = 2002:2019))

reconstruct(anal.dat.multi, sel.lower  = 1.5, sel.upper = 5, lvl=1)
