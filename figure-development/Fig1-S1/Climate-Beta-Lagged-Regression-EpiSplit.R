...
temp.df <- cbind.data.frame(x=20:34, beta_coeff=c(predict.gam(gam1, newdata = test.df, exclude = c("s(precip_mm_lag)", "s(provname)"), type = "terms")))
temp.df$beta_coeff_low <- c(predict.gam(gam1, newdata = test.df, exclude = c("s(precip_mm_lag)", "s(provname)"), type = "terms")-1.96*predict.gam(gam1, newdata = test.df, exclude = c("s(precip_mm_lag)", "s(provname)"), type = "terms", se.fit = TRUE)$se.fit)
temp.df$beta_coeff_high <-c(predict.gam(gam1, newdata = test.df, exclude = c("s(precip_mm_lag)", "s(provname)"), type = "terms")+1.96*predict.gam(gam1, newdata = test.df, exclude = c("s(precip_mm_lag)", "s(provname)"), type = "terms", se.fit = TRUE)$se.fit)
...
precip.df <- cbind.data.frame(x=0:30, beta_coeff=c(predict.gam(gam1, newdata = test.df, exclude = c("s(temp_C_lag)", "s(provname)"), type = "terms")))
precip.df$beta_coeff_low <- c(predict.gam(gam1, newdata = test.df, exclude = c("s(temp_C_lag)", "s(provname)"), type = "terms")-1.96*predict.gam(gam1, newdata = test.df, exclude = c("s(temp_C_lag)", "s(provname)"), type = "terms", se.fit = TRUE)$se.fit)
precip.df$beta_coeff_high <-c(predict.gam(gam1, newdata = test.df, exclude = c("s(temp_C_lag)", "s(provname)"), type = "terms")+1.96*predict.gam(gam1, newdata = test.df, exclude = c("s(temp_C_lag)", "s(provname)"), type = "terms", se.fit = TRUE)$se.fit)
