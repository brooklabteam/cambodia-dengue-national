...
#and load climate date to combine
...
head(dat.prov)
head(dat.nat)
dat.nat <- dplyr::select(dat.nat, -(epimonth_num))
names(dat.nat)[names(dat.nat)=="cases"] <- "national_cases"

dat.all <- merge(dat.nat, dat.prov, all.x = T,  by="epimonth")
head(dat.all)

#and temp, precip by prov plus oni and mei at nat
names(dat.all)[names(dat.all)=="epimonth"]<-"month_date"

precip.dat <- dplyr::select(precip.dat, month_date, adm1_name, precip_mm)
names(precip.dat)[names(precip.dat)=="adm1_name"] <- "provname"
dat.all$provname[dat.all$provname=="Otdar Meanchey"] <- "Oddar Meanchey"    
dat.all$provname[dat.all$provname=="Siemreap"] <- "Siem Reap"   
precip.dat$provname[precip.dat$provname=="Siemreap"] <- "Siem Reap"
setdiff(unique(precip.dat$provname), unique(dat.all$provname))

dat.all <- merge(dat.all, precip.dat, all.x = T, by=c("month_date", "provname"))
head(dat.all)

temp.dat <- dplyr::select(temp.dat, month_date, adm1_name, temp_C)
names(temp.dat)[names(temp.dat)=="adm1_name"] <- "provname"
temp.dat$provname[temp.dat$provname=="Siemreap"] <- "Siem Reap"
setdiff(unique(temp.dat$provname), unique(dat.all$provname))

dat.all <- merge(dat.all, temp.dat, all.x = T, by=c("month_date", "provname"))
head(dat.all)

#national temp and precip and the two indixes
head(nat.temp)
nat.temp <- dplyr::select(nat.temp, month_date, temp_C)
head(precip.nat)
precip.nat <- dplyr::select(precip.nat, month_date, precip_mm)
names(nat.temp) <-  c("month_date", "temp_C_national")
names(precip.nat) <- c("month_date", "precip_mm_national")
dat.all <- merge(dat.all, nat.temp, all.x = T, by="month_date")
dat.all <- merge(dat.all, precip.nat, all.x = T, by="month_date")
head(dat.all)
...
dat.all <- merge(dat.all, oni, all.x = T,  by="month_date")
dat.all <- merge(dat.all, mei, all.x = T, by="month_date")
head(dat.all)

write.csv(dat.all, file = paste0(homewd, "/data/all_case_climate.csv"), row.names = F)
